...
class Accleration_Dataset():
    def __init__(self, directory, max_time=300, num_samples = 100000, seq_len = 2000):
        ...
        self.data_x = self._load_data_X()
        self.data_y = self._load_data_Y()
        self.data_z = self._load_data_Z()
        pass

    def _load_data_X(self):
        '''
        '''
        data = []
    ...
    def _load_data_Y(self):
        ...
        for file in self.file_list:
            ...
            df = pd.read_csv(file_path)
            time = df['Time (s)'].values
            acceleration = df['Acceleration y (m/s^2)'].values
    ...
    def _load_data_Z(self):
        ...
        for file in self.file_list:
            ...
            df = pd.read_csv(file_path)
            time = df['Time (s)'].values
            acceleration = df['Acceleration z (m/s^2)'].values
