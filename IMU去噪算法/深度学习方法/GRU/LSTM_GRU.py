...
class LSTM_GRU(nn.Module):
    def __init__(self, data, h_size_list,step=20):
        ...
        self.GRU_list = [] # 存放GRU单元
        for i in range(self.step):
            self.lstm_list.append(nn.LSTM(data.shape[2], h_size_list[i], batch_first = True)) # 存放step个单元 每个单元的输入是i_size 输出是h_size_list[i]
        for i in range(self.step):
            self.GRU_list.append(nn.GRU(data.shape[2], h_size_list[i], batch_first = True)) # 存放step个单元 每个单元的输入是i_size 输出是h_size_list[i]
    ...
    def forward(self, initial_state):
        ...
        for i in range(len(x)): # len(x) = N-step
            for t in range(step):
                ...
                grumodule = self.GRU_list[t]
                y, (h_n, c_n) = lstmmodule(x[i][:,t,:].unsqueeze(-1), (h_n, c_n)) #每个时间点的数据都要经过step个lstm单元
                y, h_n_gru = grumodule(h_n.squeeze(0).unsqueeze(-1), h_n_gru) #每个时间点的数据都要经过step个gru单元
            out.append(self.fc(h_n_gru).squeeze(0)) #获取最后一个单元的输出 shape(bs,1)
    ...
    def train1(self, initial_state, epoah, lr):
        ...
        output_file_path = "./output_GPU.csv"
        # 保存最小损失时的输出到CSV文件
