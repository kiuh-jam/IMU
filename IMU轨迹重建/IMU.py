...
class CsvImuFileLoader：从csv文件中提取IMU数据 包括加速度和角速度文件路径 采样率 时间索引 加速度索引 角速度索引 以及是否插值
...
class IMU:
    ...
    存储IMU测量数据和时间信息
    '''
    def __init__(self, time:int = 0.0, dacc:np.ndarray = np.zeros(3), dgyro:np.ndarray = np.zeros(3), dt:float = 0.0):
        self.time = time # 时间(s)
...
class CsvImuFileLoader:
    ...
    def __init__(self, 
        ...
        if not self.interpolate: # 不插值
            # 初始化文件读取器 并同步时间
            self.accel_reader,self.gyro_reader = self._init_reader(acc_filepath, gyro_filepath)
            ...
            # 使用插值
            self.acc_all_data, self.gyro_all_data, self.time_all_data = self._interpolate_all_data()
            self.interp_index = 0
        pass

    def _init_reader(self, acc_filepath:str, gyro_filepath:str) -> [csv.reader,csv.reader]:
        '''
        初始化csv文件读取器
        acc_filepath: 加速度数据文件路径
        gyro_filepath: 角速度文件路径
        ...
        return acc_reader, gyro_reader # 返回加速度和角速度的csv文件读取器
    ...
    def get_next_imu(self) -> Optional[IMU]:
        ...
        self.current_IMU = IMU( acc_time, 
                                np.array([float(acc_row[i]) for i in self.acc_col]), 
                                np.array([float(gyro_row[i]) for i in self.gyro_col]), 
                                self.dt )
        self.next_accel = self._read_next(self.accel_reader)
        self.next_gyro = self._read_next(self.gyro_reader)
        return self.current_IMU

    def _interpolate_all_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        '''
        加速度和角速度大部分时间不同步时，进行插值
        插值所有数据
        '''
        ...
        all_times = np.union1d(acc_time, gyro_time)
        ...
        return acc_data, gyro_data, all_times

    def _get_next_interpolated(self) -> Optional[IMU]:
        if self.interp_index >= len(self.time_all_data):
            return None
        imu = IMU(
            self.time_all_data[self.interp_index],
            self.acc_all_data[self.interp_index],
            self.gyro_all_data[self.interp_index],
            self.dt
        )
        self.interp_index += 1
        return imu
